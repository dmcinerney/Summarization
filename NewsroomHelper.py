...
class NewsroomDataset_word2vec(VariableLength):
    ...
    def prepare_inputs(self, v_args, nv_args, lengths):
        ...
        text, text_length = get_text_indices(text, self.word_indices, text_length_max)
        summary, summary_length = get_text_indices(summary, self.word_indices, summary_length_max)
        return dict(text=text, text_length=text_length, summary=summary, summary_length=summary_length)
